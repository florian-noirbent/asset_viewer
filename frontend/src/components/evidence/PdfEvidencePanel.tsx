...
function OpenPdfEvidencePanel({ evidence, onClose }: { evidence: EvidenceTarget; onClose: () => void }) {
  ...
  return (
    <aside
      ...
      aria-modal="true"
      className="fixed inset-y-0 right-0 z-40 flex w-full max-w-[min(96vw,1100px)] flex-col border-l border-line bg-white shadow-xl lg:w-[88vw]"
      role="dialog"
    >
      ...
      <div className="flex min-h-0 flex-1 flex-col bg-slate-100">
        <div className="border-b border-line bg-white px-2 py-1">
          <Toolbar />
        </div>
...
function StatusMessage({ status, sourcePage }: { status: StatusState; sourcePage?: number }) {
  ...
  if (status.status === "loading") {
    return (
      <div className="flex items-center gap-2 border-b border-line bg-blue-50 px-5 py-2 text-sm text-blue-700">
  ...
  if (status.status === "highlighted") {
    return (
      <div className="flex items-center gap-2 border-b border-line bg-emerald-50 px-5 py-2 text-sm text-emerald-700">
  ...
  if (status.status === "page-not-found" || status.status === "document-not-found") {
    return (
      <div className="flex items-center gap-2 border-b border-line bg-amber-50 px-5 py-2 text-sm text-amber-700">
